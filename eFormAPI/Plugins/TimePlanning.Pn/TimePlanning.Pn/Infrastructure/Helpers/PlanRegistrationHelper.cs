...
public static class PlanRegistrationHelper
{
        ...
        )
    {
        ...
        foreach (var plan in planningsInPeriod)
        {
            ...
            planningModel.PauseMinutes += FlexChain.AggregatePauseMinutes(planRegistration, rowIsOneMinute);
        ...
        )
    {
        ...
        // see OneMinuteModeTimeline. The timeline is built ONCE by the calling
        // loop and passed in, so this never costs a query per row.
        ...
        PlanRegistration source)
    {
        for (var shift = 1; shift <= 5; shift++)
        {
            var overrideMinutes = FlexChain.GetShiftPauseOverrideMinutes(source, shift);
    ...
    public static void ComputeTimeTrackingFields(PlanRegistration planRegistration)
    {
        ...
        for (var shift = 1; shift <= 5; shift++)
        {
            totalPauseSeconds += FlexChain.ComputeShiftPauseSeconds(planRegistration, shift, useOneMinuteIntervals: false);
        }
