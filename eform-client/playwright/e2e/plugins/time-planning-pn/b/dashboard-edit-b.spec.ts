  ...
  test('should edit time registration in last week', async ({ page }) => {
    ...
    for (let i = 0; i < secondUpdatePlanTexts.length; i++) {
      ...
      if (secondUpdateActualTexts[i].start1StartedAt !== '' && secondUpdateActualTexts[i].start1StartedAt !== '00:00') {
        ...
        if (degrees0 > 360) {
          await page.locator('[style="height: 85px; transform: rotateZ(' + degrees0 + 'deg) translateX(-50%);"] > span').click();
        } else {
          if (degrees0 === 0) {
      ...
      await expect(page.locator('[data-testid="start1StartedAt"]')).toHaveValue(secondUpdateActualTexts[i].start1StartedAt);
      ...
      await expect(page.locator('[data-testid="stop1StoppedAt"]')).toHaveValue(secondUpdateActualTexts[i].stop1StoppedAt);
      ...
      if (secondUpdateActualTexts[i].pause1Id !== '' && secondUpdateActualTexts[i].pause1Id !== '00:00') {
        ...
        await page.locator('.timepicker-button span').filter({ hasText: 'Ok' }).click();
        await expect(page.locator('[data-testid="pause1Id"]')).toHaveValue(secondUpdateActualTexts[i].pause1Id);
      } else {
        await expect(page.locator('[data-testid="pause1Id"]')).toHaveValue('');
      }

      // Set start2StartedAt
      if (secondUpdateActualTexts[i].start2StartedAt !== '' && secondUpdateActualTexts[i].start2StartedAt !== '00:00') {
        await page.locator('[data-testid="start2StartedAt"]').click();
      ...
      await expect(page.locator('[data-testid="start2StartedAt"]')).toHaveValue(secondUpdateActualTexts[i].start2StartedAt);

      // Set stop2StoppedAt
      if (secondUpdateActualTexts[i].stop2StoppedAt !== '' && secondUpdateActualTexts[i].stop2StoppedAt !== '00:00') {
        await page.locator('[data-testid="stop2StoppedAt"]').click();
      ...
      await expect(page.locator('[data-testid="stop2StoppedAt"]')).toHaveValue(secondUpdateActualTexts[i].stop2StoppedAt);

      // Set pause2Id
      if (secondUpdateActualTexts[i].pause2Id !== '' && secondUpdateActualTexts[i].pause2Id !== '00:00') {
        await page.locator('[data-testid="pause2Id"]').click();
        ...
        await page.locator('.timepicker-button span').filter({ hasText: 'Ok' }).click();
        await expect(page.locator('[data-testid="pause2Id"]')).toHaveValue(secondUpdateActualTexts[i].pause2Id);
      } else {
        await expect(page.locator('[data-testid="pause2Id"]')).toHaveValue('');
      }

      // Verify calculated values
      await expect(page.locator('#flexToDate')).toHaveValue(secondUpdateActualTexts[i].flexToDate);
      await expect(page.locator('#flexIncludingToday')).toHaveValue(secondUpdateActualTexts[i].flexIncludingToday);
      await expect(page.locator('#nettoHours')).toHaveValue(secondUpdateActualTexts[i].nettoHours);
      await expect(page.locator('#todaysFlex')).toHaveValue(secondUpdateActualTexts[i].todaysFlex);
      await expect(page.locator('#paidOutFlex')).toHaveValue(secondUpdateActualTexts[i].paidOutFlex.toString());
      await expect(page.locator('#planHours')).toHaveValue(secondUpdateActualTexts[i].calculatedHours.toString());
