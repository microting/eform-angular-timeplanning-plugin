...
/**
 * j1m variant of `j/dashboard-edit-a.spec.ts` — the "validate global values
 * setting" test, exercised under the `UseOneMinuteIntervals = true` code
 * path. The shard's `post-migration.sql` flips the flag on every active
 * assigned site, so the legacy in-spec setup from `j/` runs against the
 * `minutesGap = 1` timepicker context.
 *
 * Why "no time picker fills" shape (deviation from b1m / c1m / d1m / e1m / f1m):
 *
 *   The legacy `j/` shard validates auto-break calculation global values
 *   round-tripping — it captures `LoadDefaults`-populated readonly inputs
 *   (computed values), saves, and verifies they re-render identically on
 *   reopen. NO planned-shift time-pickers are touched. There is therefore
 *   no off-grid value to inject; the flag-on minute-granularity rendering
 *   is exercised solely by the dialog open/close + tab switching paths.
 *   Cloning in the same shape (no time-picker fills, no shift assertions)
 *   is the correct call — forcing a multishift block here would time out
 *   on `[data-testid="plannedStartOfShift1"]` exactly the way an earlier
 *   (reverted) g1m attempt did.
 *
 * Save gating: the legacy `j` spec uses `force: true` on `#saveButton`
 * inside the dialog (the button is always enabled in this flow because
 * the form is dirty after `LoadDefaults` clicks). We deliberately keep
 * the same shape as source — this isn't a planned-shift edit so the
 * `expect(...).toBeEnabled(...)` BEFORE-`click()` discipline from the
 * b1m..i1m specs doesn't apply identically here.
 *
 * Display assertions: equality compares the captured input values from
 * the first dialog against re-render values from the second dialog,
 * keyed by element id. No time-format string is asserted, so the
 * `HH:mm` vs `HH:mm:ss` distinction is irrelevant here.
 */
...
test.describe('Dashboard edit values (j1m, flag-on, global values round-trip)', () => {
