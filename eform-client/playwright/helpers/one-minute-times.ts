...
/**
 * h1m shard variant: paid-out-flex decimal-input round-trip tests cloned from
 * `h/dashboard-edit-a.spec.ts`. Where b1m / c1m / d1m / e1m sweep different
 * clock quadrants with five-shift round-trip data, h1m's tests revolve around
 * a NUMERIC field (`#paidOutFlex`) — its time inputs are incidental but the
 * workday-entity dialog's `plannedShiftDurationValidator` requires shift 1 to
 * have a valid (start < stop) pair before `#saveButton` becomes enabled. To
 * stay consistent with the multishift-shape pattern we fill ALL FIVE shifts
 * ascending (off-grid) so the spec exercises the `minutesGap=1` picker on
 * every shift slot, not just shift 1.
 *
 * The four paid-out-flex tests rely on save-state carrying across tests
 * (`should accepts decimal values with dot` reads back `'1.2'` saved by the
 * previous test), so the same five-shift block is reused unchanged in each
 * test's beforeEach via `setShift()`.
 *
 * Clock-quadrant coverage: h1m parks in late-morning to late-evening
 * (11-22) — straddles the outer→inner ring boundary at hour 12 and exits
 * back through the inner ring at hours 13-22. Distinct from b1m (08-16),
 * c1m (08-19), d1m (13-23), e1m (01-11), and the (reverted) g1m (10-19) so
 * the variant matrix as a whole sweeps the full clock surface across all 1m
 * shards. Every value is non-aligned to 5 minutes so the flag-on
 * `minutesGap=1` rendering is the only way the picker can land on these
 * values.
 *
 * Same shift-order constraint as `OFFGRID_TIMES` above:
 * shift{n+1}.start MUST be >= shift{n}.stop.
 */
export const OFFGRID_TIMES_H1M = {
  shift1Start: '11:02',
  shift1End:   '12:14',
  shift2Start: '12:26',
  shift2End:   '14:38',
